...
class RbbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('rbb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `rbb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM rbb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO rbb (date, title, link, content)"
